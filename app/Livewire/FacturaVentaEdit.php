...
class FacturaVentaEdit extends Component
{
    public $notas_envio;
    ...
    {
        $this->fechaEmision = $this->factura_venta->FechaEmision;
        $this->fechaVencimiento = $this->factura_venta->FechaVencimiento;
        $this->pto_ventas = PuntoDeVenta::all();
        $this->condiciones_venta = CondicionVenta::all();

        $condicionesFactura = $this->factura_venta->CondicionVenta;

        $condicionesDesdeFactura = collect(explode('/', $condicionesFactura))
            ->map(fn($c) => trim($c))
            ->filter()
            ->toArray();

        $this->condicionesSeleccionadas = $condicionesDesdeFactura;

        $this->ordenarCondiciones();

        $this->notas_envio = $this->factura_venta->itemsFacturaVenta;

        $this->next_factura_numero = $this->factura_venta->Numero;
    ...
    {
        return view('livewire.factura-venta-edit');
    }
}
