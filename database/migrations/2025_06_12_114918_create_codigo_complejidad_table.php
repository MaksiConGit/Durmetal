<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('codigo_complejidad', function (Blueprint $table) {
            $table->id();
            $table->foreignId('IdTratamiento')->constrained('tratamiento')
                                                    ->onDelete('restrict')
                                                    ->onUpdate('cascade');
            $table->string('Descripcion');
            $table->float('Precio');
            $table->string('Divisa');
            $table->float('PorcentajeCoeficiente');
            $table->float('Coeficiente');
            $table->integer('CC');
            $table->date('FechaCreacion')->nullable();
            $table->foreignId('CreadoPor')->nullable()->constrained('users')
                                                        ->onDelete('restrict')
                                                        ->onUpdate('cascade');
            $table->date('FechaActualizacion')->nullable();
            $table->foreignId('ActualizadoPor')->nullable()->constrained('users')
                                                            ->onDelete('restrict')
                                                            ->onUpdate('cascade');
            $table->boolean('Activo')->nullable();
            $table->string('IdTratamientoCodigoComplejidad');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('codigo_complejidad');
    }
};
